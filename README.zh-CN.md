...
- **计费周期** —— 套餐 id、订阅状态、当前周期（开始 → 结束）。
- **本地费用（图表化）** —— 可选 今日 / 本周 / 本月 / 全部。本周是**堆叠柱状图**（每天费用、按模型分色并配图例）；本月是**日历热力格**（每天费用，强度按当月最大值缩放）；每个区间下方都有**横向模型排行**，从多到少。token 一律以 `M`（百万）为单位显示。
- **累计用量** —— 本计费周期的请求次数、成功率与 token 总量。
